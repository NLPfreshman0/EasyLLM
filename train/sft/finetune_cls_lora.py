...
class SaveScoreLayerCallback(TrainerCallback):
    def on_save(
        self,
        args: TrainingArguments,
        state: TrainerState,
        ...
        **kwargs,
        ...
        if kwargs['model'] is not None:
            ...
            score_layer = kwargs['model'].score  
            score_layer_weights = score_layer.state_dict()
            save_path = os.path.join(checkpoint_folder, "score_layer_weights.bin")
            torch.save(score_layer_weights, save_path)
            print(f"Saved 'score' layer weights to {save_path}")
            return control
...
def main():
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            output = tokenizer([ item for item in examples[text_column_name]],truncation=True,max_length=data_args.block_size,padding=True,return_tensors=None)
            output['labels'] = output['input_ids'].copy()
        return output

    def tokenize(prompt):
        result = tokenizer(prompt,truncation=True,max_length=data_args.block_size,padding='max_length',return_tensors=None)
        result["labels"] = result["input_ids"].copy()
        return result

    def generate_and_tokenize_prompt(data_point):
        input_text = data_point[input_column_name]
        target = data_point[target_column_name]
        tokenized_full_prompt = tokenize(input_text)
        tokenized_full_prompt['labels'] = target
    ...
    trainer = Trainer(
        ...
        callbacks=([SavePeftModelCallback] if isinstance(model, PeftModel) else None) + ([SaveScoreLayerCallback]),
    )
